                        ...
                        <form action="{{ route('student.update', $student->id) }}" method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control"
                                       name="nim" value="{{ old('nim', $student->nim) }}" readonly>

                                <div class="form-text">
                                    *hanya diubah apabila mahasiswa pindah program studi
                                </div>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="enroll_year" id="enroll_year"
                                    ...
                                    @for($year = 2018; $year <= 2030; $year++)
                                        <option value="{{ $year }}" {{ old('enroll_year', $student->enroll_year) == $year ? 'selected':'' }}>{{ $year }}</option>
                                    @endfor
                            ...
                            <div class="mb-3">
                                ...
                                <select name="enroll_semester" id="enroll_semester"
                                    ...
                                    @for($semester = 1; $semester <= 14; $semester++)
                                        <option value="{{ $semester }}" {{ old('enroll_semester', $student->enroll_semester) == $semester ? 'selected':'' }}>{{ $semester }}</option>
                                    @endfor
                            ...
                            <div class="mb-3">
                                ...
                                <select name="study_program_id" id="study_program_id"
                                    ...
                                    @forelse($programs as $program)
                                        <option value="{{ $program->id }}" {{ old('study_program_id', $student->study_program_id) == $program->id ? 'selected':'' }}>{{ $program->name }}</option>
                                    @empty
